 ...
 *
 */
class TransportDocumentAction extends TransportCommonAction{
	// -------- 文档资料->侧边栏 --------
	public function document_sidebar() {
		layout( './Common/frame' );
		$this->display( './Public/html/Content/Transport/document/document_sidebar.html' );
	}
	...
	public function law() {
		...
		// if ( $document ) {
			...
			$tmp_content=$this->fetch( './Public/html/Content/Transport/document/law.html' );
	...
	public function manual() {
		...
		// if ( $document ) {
			$document_json = json_encode( $document );
			$tmp_content=$this->fetch( './Public/html/Content/Transport/document/manual.html' );
			$tmp_content = "<script> document_json=$document_json; </script> $tmp_content";
			$this->ajaxReturn( $tmp_content );
		//} else {
			$this->ajaxReturn( '加载页面失败，请重新点击侧边栏加载页面。' );
	...
	public function announcement() {
		...
		// if ( $document ) {
			$document_json = json_encode( $document );
			$tmp_content=$this->fetch( './Public/html/Content/Transport/document/announcement.html' );
			$tmp_content = "<script> document_json=$document_json; </script> $tmp_content";
			$this->ajaxReturn( $tmp_content );
		//} else {
			$this->ajaxReturn( '加载页面失败，请重新点击侧边栏加载页面。' );
