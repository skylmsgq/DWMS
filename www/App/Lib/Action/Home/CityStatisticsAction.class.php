...
class CityStatisticsAction extends CityCommonAction{
	...
	public function direction_statistic(){
		$manifest = M('manifest')->join('reception_unit ON manifest.reception_unit_id = reception_unit.reception_unit_id')->join('jurisdiction ON reception_unit.jurisdiction_id = jurisdiction.jurisdiction_id')->where( array( 'jurisdiction_up_id' => session('jurisdiction_id') ) )->select();
	...
	public function manifest_statistic(){
		$manifest = M('manifest')->join('reception_unit ON manifest.reception_unit_id = reception_unit.reception_unit_id')->join('jurisdiction ON reception_unit.jurisdiction_id = jurisdiction.jurisdiction_id')->where( array( 'jurisdiction_up_id' => session('jurisdiction_id') ) )->select();
		$record = M('record')->join('reception_unit ON record.reception_unit_id = reception_unit.reception_unit_id')->join('jurisdiction ON reception_unit.jurisdiction_id = jurisdiction.jurisdiction_id')->where( array( 'jurisdiction_up_id' => session('jurisdiction_id') ) )->select();
		...
		$delete_manifest_num=M('manifest')->join('production_unit ON production_unit.production_unit_id = manifest.production_unit_id')->join('jurisdiction ON production_unit.jurisdiction_id = jurisdiction.jurisdiction_id')->where( array( 'jurisdiction_up_id' => session('jurisdiction_id'),'manifest_status' => -1 ) )->count();
		$delete_record_num=M('record')->join('production_unit ON production_unit.production_unit_id = record.production_unit_id')->join('jurisdiction ON production_unit.jurisdiction_id = jurisdiction.jurisdiction_id')->where( array( 'jurisdiction_up_id' => session('jurisdiction_id'),'record_status' => -1 ) )->count();
	...
	public function enterprise_statistic(){
		...
		$rfid = M('manifest')->join( 'production_unit ON manifest.production_unit_id = production_unit.production_unit_id' )->join('jurisdiction ON production_unit.jurisdiction_id = jurisdiction.jurisdiction_id')->where( array( 'jurisdiction_up_id' => session('jurisdiction_id') ) )->select();
