 ...
 *	P
 */
class ProvinceMapAction extends ProvinceCommonAction{
	// -------- 转移地图 侧边栏 --------
	public function map_sidebar() {
		layout( './Common/frame' );
		$this->display( './Public/html/Content/Province/map/map_sidebar.html' );
	}
	...
	public function transfer_map_display() {
		...
		$condition['jurisdiction_up_id_2'] = array( 'EQ', session( 'jurisdiction_id' ) );
		...
		$alarm_distance = M( 'alarm_distance' )->join('jurisdiction ON alarm_distance.jurisdiction_id = jurisdiction.jurisdiction_id')->where( array( 'jurisdiction_up_id_2' => session( 'jurisdiction_id' ) ) )->find();
		...
		if ( $route_vehicle_join && $alarm_distance ) {
			...
			$tmp_content = $this->fetch( './Public/html/Content/Province/map/transfer_map_display.html' );
	...
	public function designate_vehicle_playback() {
		...
		if ( $transport_unit && $alarm_distance ) {
			...
			$alarm_distance_json = json_encode( $alarm_distance );
			$tmp_content = $this->fetch( './Public/html/Content/Province/map/designate_vehicle_playback.html' );
	...
	public function transfer_map_playback() {
		...
		if ( $alarm_distance ) {
			$alarm_distance_json = json_encode( $alarm_distance );
			$tmp_content = $this->fetch( './Public/html/Content/Province/map/transfer_map_playback.html' );
	...
	public function warehouse_map_display() {
		...
		if ( ( $production_unit ) && ( $reception_unit ) ) {
			...
			$tmp_content = $this->fetch( './Public/html/Content/Province/map/warehouse_map_display.html' );
	...
	public function transfer_route_plan() {
		...
		if ( ( $production_unit ) && ( $reception_unit ) ) {
			$production_unit_json = json_encode( $production_unit );
			$reception_unit_json = json_encode( $reception_unit );
			$tmp_content=$this->fetch( './Public/html/Content/Province/map/transfer_route_plan.html' );
			$tmp_content="<script> production_unit_json=$production_unit_json; reception_unit_json=$reception_unit_json; </script> $tmp_content";
			$this->ajaxReturn( $tmp_content );
		} else {
			$this->ajaxReturn( "加载页面失败，请重新点击侧边栏加载页面。" );
	...
	public function transfer_route_query() {
		$production_unit = M( 'production_unit' )->join('jurisdiction ON production_unit.jurisdiction_id = jurisdiction.jurisdiction_id')->where( array( 'jurisdiction_up_id_2' => session( 'jurisdiction_id' ) ) )->select();
		$reception_unit = M( 'reception_unit' )->join('jurisdiction ON reception_unit.jurisdiction_id = jurisdiction.jurisdiction_id')->where( array( 'jurisdiction_up_id_2' => session( 'jurisdiction_id' ) ) )->select();
		$production_unit_json = json_encode( $production_unit );
		$reception_unit_json = json_encode( $reception_unit );
		if ( $production_unit_json && $reception_unit_json ) {
			$tmp_content=$this->fetch( './Public/html/Content/Province/map/transfer_route_query.html' );
			$tmp_content = "<script> production_unit_json=$production_unit_json; reception_unit_json=$reception_unit_json; </script> $tmp_content";
			$this->ajaxReturn( $tmp_content );
		} else {
			$this->ajaxReturn( 'fail' );
	...
	public function gps_data_simulator() {
		...
		if ( $device ) {
			...
			$tmp_content = $this->fetch( './Public/html/Content/Province/map/gps_data_simulator.html' );
